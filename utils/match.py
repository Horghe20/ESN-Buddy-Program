...
This module provides functions to compute matching scores between Buddies and Esners
...
- database.tables: Contains the Buddy and Esners models.
...
- match_making(buddies, esners): Performs matchmaking between lists of Buddy and Esner instances.
...
import numpy as np
# from sentence_transformers import SentenceTransformer  # Unused here, but kept if needed later.
from sklearn.metrics.pairwise import cosine_similarity

from database.tables import Buddy, Esners
from utils.config import TOP_AUTOMATIC_MATCH

def compute_match_score(buddy: Buddy, esner: Esners, weights=None):
    """
    ...
    :param esner: An instance of the Esners model representing an ESN associate.
...
def match_making(buddies, esners):
    ...
    2. **Candidate Selection**:
       - For each Buddy, a list of candidate Esners is built. Each candidate is a tuple containing:
         (esner.id, match_score, current_buddy_count, index_in_esners_list).
    ...
    3. **Sorting of Candidates**:
       - The candidates are sorted using two criteria:
         a) **Primary:** The current number of buddies (ascending order, so that Esners with fewer buddies are prioritized).
    ...
    **Example**:
    ...
    # Process each buddy to find the best matching Esners.
    for i, buddy in enumerate(buddies):
        buddy_id = buddy.id
        ...
        # Sort candidates: prioritize Esners with fewer buddies and higher match score.
...
# esners = [esner1, esner2, ...]     # SQLAlchemy model instances of Esners.
#
# matched_data = match_making(buddies, esners)
# for buddy, esner, score in matched_data:
#     print(f"Buddy {buddy.id} matched with Esner {esner.id} with score {score}%")
